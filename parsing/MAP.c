...
/*   Updated: 2025/03/27 18:12:33 by abelmoha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill( t_game *game, char **map_copy, t_point current)
{
	if (current.x < 0 || current.y < 0 || current.x >= (double)ft_strlen(map_copy[(int)current.y]) || 
		...
		return ;
	if (map_copy[(int)current.y][(int)current.x] == ' ' || (((int)current.x == 0 || (int)current.x == (int)ft_strlen(map_copy[(int)current.y]) - 1 ||
